...
import { feedService, FeedJob, JOB_SOURCES } from "../services/feedService";
...
export default function JobFeedScreen({ navigation }: any) {
  ...
  useFocusEffect(useCallback(() => {
    loadCached().then(async () => {
      const last = await feedService.getLastFetchTime();
      const stale = !last || Date.now() - last.getTime() > 30 * 60 * 1000; // 30 min
      if (stale) fetchFresh();
    });
  }, []));
  ...
  const captureJob = (job: FeedJob) => {
    navigation.navigate("Capture", {
      screen: "JobCapture",
      params: { prefillUrl: job.url, prefillTitle: job.title, prefillCompany: "" },
    });
  };
...
const styles = StyleSheet.create({
  ...
  loadingContainer: { flex: 1, backgroundColor: theme.colors.bg.primary, paddingTop: 80, paddingHorizontal: theme.spacing.md },
  ...
  sourcesList: { marginTop: theme.spacing.lg, gap: theme.spacing.sm },
