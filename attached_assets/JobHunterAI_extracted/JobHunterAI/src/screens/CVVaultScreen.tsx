...
import { db, CVVault } from "../services/storage";
...
export default function CVVaultScreen() {
  ...
  const restoreVersion = (cvText: string, vLabel: string) => {
    Alert.alert(
      "Restore Version",
      ...
      [
        { text: "Cancel", style: "cancel" },
        {
          text: "Restore",
          onPress: async () => {
            await db.saveCVVault(cvText, vLabel);
            await load();
      ...
      {(!hasCV || editing) && (
        <View style={styles.editorContainer}>
          <View style={styles.field}>
            <Text style={styles.label}>CV Label</Text>
            <TextInput
              style={styles.input}
          ...
          <View style={styles.field}>
            <Text style={styles.label}>Paste Your CV Text</Text>
...
Soil analysis, Fertilizer recommendations..."
          ...
          <View style={styles.btnRow}>
            {editing && (
              <TouchableOpacity style={styles.cancelBtn} onPress={() => { setEditing(false); setCvText(vault?.cvText || ""); }}>
      ...
      {hasCV && !editing && (
        <View style={styles.previewContainer}>
          <Text style={styles.previewTitle}>Current CV</Text>
...
const styles = StyleSheet.create({
  ...
  header: { flexDirection: "row", justifyContent: "space-between", alignItems: "center", paddingHorizontal: theme.spacing.md, paddingTop: 60, paddingBottom: theme.spacing.md },
