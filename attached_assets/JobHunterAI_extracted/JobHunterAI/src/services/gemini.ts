// ============================================
// AI SERVICE - Powered by Google Gemini (FREE)
// Get your free API key at: aistudio.google.com
// ============================================

import { CONFIG } from "../config";

const GEMINI_API = `https://generativelanguage.googleapis.com/v1beta/models/gemini-2.0-flash:generateContent?key=${CONFIG.GEMINI_API_KEY}`;
...
export const aiService = {
  // Parse job description from text or URL content
...
`);
  ...
  // Generate cover letter with tone and length controls
...
`);
  ...
  async tailorCVPoints(jobDescription: string): Promise<string> {
    return callGemini(`
Given this job description, generate tailored CV content for Wesley Kipkemoi Koech.
...
[keywords in the JD that Wesley should add]
`);
  },
...
`);
  ...
  // Answer specific application form questions
...
Return this exact JSON:
{
  ...
  // Score job relevance for Wesley's profile (batch)
